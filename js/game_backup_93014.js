...
function preload() {
    ...
    game.load.image('hittah','assets/hit.png');
...
function create() {
    ...
    hitbox = game.add.sprite(96, 0, 'hittah');
    football = game.add.sprite(160, 820, 'football');

    game.physics.enable([football, leftUprights, rightUprights, hitbox, grass, grass2], Phaser.Physics.ARCADE);
...
function update() {
    ...
    if (launched == true) {
        ...
        //keepScore.y = game.camera.y;
    }
    
    if ((football.body.velocity.y < 0 && football.body.velocity.y > -1)) {
       // alert("Try again?");   
    }
